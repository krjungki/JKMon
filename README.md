...
CPU와 메모리를 숫자로 표시하면 어느 쪽이 어느 값인지 알 수 없으므로 숫자 위에 `CPU`, `Memory` 이름을 붙인다.
크기는 **Caption size**로 조절하고 0으로 두면 숨긴다. 막대와 파이는 모양과 색으로 구분되므로 이름을 붙이지 않는다.

상단 캡션은 반투명 배경 밖에 놓여 바탕화면 위에 바로 뜨다. 그래서 밝은 배경에서도 읽힐 수 있도록 **Caption shadow**를
따로 둔다. 본문 글자의 Text shadow와 독립이다.

**마우스가 올라오면 사라지기**를 켜면 포인터가 오버레이 영역에 들어가 있는 동안 오버레이가 투명해지고, 벗어나면
다시 나타난다. 항상 위로 띄워 둘 때 아래 내용을 가리지 않게 하려는 옵션이다. 기본값은 꺼짐이다.

**Icon order**는 이 PC에서 실행 중인 클라이언트만 나열한다. Syncthing이나 Global Secure Access가 없는 PC에서는 목록에
나타나지 않고, 하나도 없으면 안내 문구만 보인다. 숨겨진 공급자의 순서는 저장된 채로 남아 나중에 설치하면 원래 자리로 돌아온다.
